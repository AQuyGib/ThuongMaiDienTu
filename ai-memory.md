...
- **Tách cấu trúc Quản lý Vòng quay (Lucky Wheel Manager) & Phần thưởng đổi điểm (Points Exchange):**
  ...
  - **Nâng cấp và Chạy Seeder Phần Thưởng (`RewardSeeder.php`)**:
    - Thiết kế lại seeder `RewardSeeder` với bộ dữ liệu phong phú, đa dạng gồm các loại Voucher giảm giá, Voucher ship và Quà tặng sản phẩm thật (như Bình giữ nhiệt, Balo, Tai nghe) cho cả hệ thống đổi điểm (Exchange) và vòng quay may mắn (Lucky Wheel).
