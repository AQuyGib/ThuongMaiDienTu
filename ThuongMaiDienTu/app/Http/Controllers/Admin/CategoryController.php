...
class CategoryController extends Controller
{
    ...
    public function update(StoreCategoryRequest $request, Category $category)
    {
        ...
        // Chống lặp vòng cha-con (vòng lặp vô hạn ở giao diện)
        if ($newParentId !== null) {
            if ((int)$newParentId === (int)$category->category_id) {
                return back()->withInput()->with('error', 'Không thể chọn chính danh mục này làm danh mục cha.');
            }

            if (Category::isDescendant($newParentId, $category->category_id)) {
                return back()->withInput()->with('error', 'Không thể chọn danh mục con làm danh mục cha.');
            }
        }
