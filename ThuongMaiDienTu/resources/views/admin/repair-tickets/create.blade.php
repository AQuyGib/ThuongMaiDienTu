...
@section('title', 'Tạo phiếu sửa chữa')
@section('page-title', 'Tạo phiếu sửa chữa')

@section('content')
<div class="p-6 space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Tạo phiếu sửa chữa</h1>
            <p class="text-sm text-gray-500">Tạo phiếu tiếp nhận thiết bị mới từ khách hàng.</p>
        </div>
        <x-ui.button
            variant="secondary"
    ...
    <form method="POST" action="{{ route('admin.repair-tickets.store') }}" class="space-y-6">
        @csrf
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm space-y-4">
            ...
            <div class="grid gap-4 md:grid-cols-3">
                ...
                <div>
                    ...
                    <input type="text" name="customer_address" value="{{ old('customer_address') }}" class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500" placeholder="Địa chỉ (tùy chọn)">
                </div>
                ...
                <div>
                    ...
                    <select name="customer_source" class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        ...
                        <option value="Facebook" {{ old('customer_source') == 'Facebook' ? 'selected' : '' }}>Facebook</option>
                        <option value="Google" {{ old('customer_source') == 'Google' ? 'selected' : '' }}>Google</option>
                        <option value="Người quen giới thiệu" {{ old('customer_source') == 'Người quen giới thiệu' ? 'selected' : '' }}>Người quen giới thiệu</option>
                        <option value="Website" {{ old('customer_source') == 'Website' ? 'selected' : '' }}>Website</option>
                        <option value="Khác" {{ old('customer_source') == 'Khác' ? 'selected' : '' }}>Khác</option>
                    </select>
                </div>
            </div>
        </div>
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm space-y-4">
            ...
            <div class="grid gap-4 md:grid-cols-2">
                ...
                <div>
                    ...
                    <input type="date" name="schedule_date" value="{{ old('schedule_date') }}" min="{{ date('Y-m-d') }}" class="w-full rounded-lg border {{ $errors->has('schedule_date') ? 'border-red-400 ring-1 ring-red-400' : 'border-gray-300' }} bg-white px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
            ...
            <div>
                ...
                <textarea name="issue_desc" rows="3" class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required placeholder="Nhập chi tiết lỗi máy...">{{ old('issue_desc') }}</textarea>
            </div>
        </div>
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm space-y-4">
            ...
            <div class="grid gap-4 md:grid-cols-3">
                <div>
                    ...
                    <input type="number" name="estimated_cost" value="{{ old('estimated_cost', 0) }}" class="w-full rounded-lg border {{ $errors->has('estimated_cost') ? 'border-red-400 ring-1 ring-red-400' : 'border-gray-300' }} bg-white px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required min="0">
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="service_fee" value="{{ old('service_fee', 0) }}" class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500" min="0">
                </div>
            </div>
