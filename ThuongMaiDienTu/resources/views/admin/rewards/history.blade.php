...
@php 
    // Lấy ngôn ngữ hiện tại của hệ thống (vi hoặc en) để hiển thị nhãn phù hợp
...
@section('content')
<div class="space-y-6">
    <!-- KHỐI 1: Tiêu đề trang (Header)
         Hiển thị tiêu đề, mô tả và nút điều hướng quay lại trang quản trị cấu hình Rewards.
    -->
    <div class="bg-white rounded-3xl p-6 shadow-sm border border-slate-200">
        <div class="flex items-center justify-between gap-4 flex-wrap">
            <div>
                <h1 class="text-2xl font-extrabold text-slate-900">
                    {{ $locale === 'en' ? 'Reward & Spin History' : 'Lịch sử đổi thưởng & Trúng giải' }}
                </h1>
                <p class="text-slate-500 mt-1">
                    {{ $locale === 'en' ? 'View and track all customer reward redemptions and lucky wheel spins.' : 'Xem và theo dõi toàn bộ lịch sử đổi voucher, quà tặng và quay thưởng của khách hàng.' }}
                </p>
            </div>
            <!-- Nút điều hướng quay lại danh sách quản trị vòng quay/quà tặng -->
    ...
    <!-- KHỐI 2: Form Tìm kiếm & Lọc dữ liệu (Filters)
         Gửi yêu cầu GET lên server với các tham số: từ khóa tìm kiếm (search), phân loại (type) và trạng thái (status).
    -->
    <form method="GET" class="bg-white rounded-3xl border border-slate-200 p-5 shadow-sm grid grid-cols-1 md:grid-cols-4 gap-4">
        <!-- Ô nhập từ khóa (Tên khách hàng, email hoặc mã code) -->
        <div>
            <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">
                {{ $locale === 'en' ? 'Search Keyword' : 'Từ khóa tìm kiếm' }}
            </label>
        ...
        <!-- Bộ lọc phân loại: Chỉ hiển thị đổi quà (redeem) hoặc quay thưởng (spin) -->
        <div>
            <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">
                {{ $locale === 'en' ? 'Reward Type' : 'Phân loại' }}
            </label>
        ...
        <!-- Bộ lọc Trạng thái: Lọc theo trạng thái chi tiết của bản ghi đổi thưởng / lượt quay -->
        <div>
            <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">
    ...
    @php
        // Bản đồ cấu hình class CSS (Badge) và nhãn hiển thị ứng với từng trạng thái của bản ghi
        $statusMap = [
    ...
    <div class="grid grid-cols-1 gap-6">
        <!-- KHỐI 3: Danh sách đổi thưởng (Chỉ hiển thị khi không lọc loại 'spin') -->
        @if(!$type || $type === 'redeem')
        <div class="bg-white rounded-3xl p-6 shadow-sm border border-slate-200">
            <!-- Header của bảng Đổi thưởng -->
            <div class="flex items-center justify-between mb-4 flex-wrap gap-2">
                ...
                <!-- Badge hiển thị tổng số dòng dữ liệu đổi thưởng -->
                <span class="px-3 py-1 rounded-full bg-indigo-50 text-indigo-700 font-extrabold text-xs border border-indigo-100/50 shadow-sm">
                    {{ $redemptions->total() }} {{ $locale === 'en' ? 'records' : 'bản ghi' }}
                </span>
            </div>
            
            <!-- Bảng hiển thị thông tin đổi thưởng -->
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse($redemptions as $item)
                            @php 
                                // Lấy badge màu và nhãn hiển thị tương ứng từ map
                                $badge = $statusMap[$item->status] ?? ['bg-slate-50 text-slate-600', $item->status]; 
                            @endphp
                            <tr class="hover:bg-slate-50/40 transition-colors">
                                <!-- Cột 1: Thông tin khách hàng (Avatar + Tên + Email) -->
                                <td class="py-4 px-4">
                                ...
                                <!-- Cột 2: Thông tin quà tặng được đổi (Ảnh + Tên + Phân loại) -->
                                <td class="py-4 px-2">
                                    <div class="flex items-center gap-2.5">
                                        <div class="w-10 h-10 rounded-lg bg-slate-100 overflow-hidden shrink-0 flex items-center justify-center border border-slate-200/50 shadow-sm">
                                            ...
                                            @else
                                                {{-- Fallback biểu tượng nếu quà tặng không có hình ảnh đại diện --}}
                                ...
                                <!-- Cột 3: Mã đổi quà (Voucher code để áp dụng khi thanh toán) -->
                                <td class="py-4 px-2">
                                ...
                                <!-- Cột 4: Điểm tích lũy tiêu tốn khi đổi món quà này -->
                                ...
                                <!-- Cột 7: Hạn sử dụng của mã đổi quà -->
                        ...
                        @empty
                            {{-- Dòng thông báo khi không tìm thấy bản ghi đổi thưởng nào khớp bộ lọc --}}
                            <tr>
            ...
            <!-- Phân trang Bootstrap cho bảng Đổi thưởng -->
            <div class="mt-4">
                {{ $redemptions->links() }}
            </div>
        </div>
        ...
        <!-- KHỐI 4: Lịch sử quay thưởng vòng quay may mắn (Chỉ hiển thị khi không lọc loại 'redeem') -->
        @if(!$type || $type === 'spin')
        <div class="bg-white rounded-3xl p-6 shadow-sm border border-slate-200">
            <!-- Header của bảng Quay thưởng -->
            <div class="flex items-center justify-between mb-4 flex-wrap gap-2">
                <h2 class="text-lg font-extrabold text-slate-800 flex items-center gap-2">
                ...
                <!-- Badge hiển thị tổng số dòng dữ liệu lượt quay -->
                <span class="px-3 py-1 rounded-full bg-fuchsia-50 text-fuchsia-700 font-extrabold text-xs border border-fuchsia-100/50 shadow-sm">
                    {{ $spins->total() }} {{ $locale === 'en' ? 'records' : 'bản ghi' }}
                </span>
            </div>
            
            <!-- Bảng hiển thị thông tin lượt quay -->
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    <thead class="text-left text-slate-600 border-b border-slate-100 bg-slate-50/50">
                        <tr>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse($spins as $spin)
                            @php 
                                // Lấy badge màu và nhãn trạng thái từ map khởi tạo phía trên
                                $badge = $statusMap[$spin->status] ?? ['bg-slate-50 text-slate-600', $spin->status]; 
                            @endphp
                            <tr class="hover:bg-slate-50/40 transition-colors">
                                <!-- Cột 1: Khách hàng thực hiện lượt quay -->
                                <td class="py-4 px-4">
                                    <div class="flex items-center gap-2.5">
                                ...
                                <!-- Cột 2: Phần quà trúng thưởng tại ô quay (Ảnh + Tên + Loại) -->
                                <td class="py-4 px-2">
                                    <div class="flex items-center gap-2.5">
                                        <div class="w-10 h-10 rounded-lg bg-slate-100 overflow-hidden shrink-0 flex items-center justify-center border border-slate-200/50 shadow-sm">
                                ...
                                <!-- Cột 3: Mã trúng thưởng ngẫu nhiên sinh ra từ vòng quay -->
                                <td class="py-4 px-2">
                                ...
                                <!-- Cột 4: Điểm tiêu tốn để mua lượt quay này -->
                                <td class="py-4 px-2 text-center">
                                    <span class="inline-flex items-center font-black text-rose-600 text-xs gap-0.5 bg-rose-50/50 px-2 py-0.5 rounded">
                                        <i class="fa-solid fa-coins text-[10px]"></i> -{{ number_format($spin->points_spent) }}
                                    </span>
                                </td>
                                <!-- Cột 5: Trạng thái (Trúng quà, Hủy, Đã dùng, v.v.) -->
                                <td class="py-4 px-2 text-center">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold border {{ $badge[0] }}">{{ $badge[1] }}</span>
                                </td>
                                <!-- Cột 6: Thời gian bấm quay số -->
                                <td class="py-4 px-2 text-xs text-slate-500 font-medium">
                                    <div class="flex items-center gap-1">
                                        <i class="fa-regular fa-clock text-slate-400 text-[10px]"></i>
                                ...
                                <!-- Cột 7: Hạn sử dụng của quà tặng trúng thưởng (nếu có) -->
                                <td class="py-4 px-4 text-xs text-slate-500 font-medium text-right">
                                    @if($spin->expires_at)
                                        <div class="inline-flex items-center gap-1 bg-amber-50/40 text-amber-700 border border-amber-100/30 px-2 py-0.5 rounded">
                                            <i class="fa-regular fa-calendar-check text-[10px]"></i>
                        ...
                        @empty
                            {{-- Dòng thông báo khi không tìm thấy lượt quay nào khớp bộ lọc --}}
                            <tr>
            ...
            <!-- Phân trang Bootstrap cho bảng Lượt quay -->
            <div class="mt-4">
                {{ $spins->links() }}
            </div>
        </div>
