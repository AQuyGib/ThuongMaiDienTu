                    ...
                    <tr class="hover:bg-gray-50 border-b border-gray-100 transition">
                        ...
                        <td class="p-4">
                            @if($user->member_tier == 'Vang')
                                <span class="bg-yellow-100 text-yellow-700 px-2 py-1 rounded text-xs font-bold"><i class="fa-solid fa-crown mr-1"></i>Vàng</span>
                            @elseif($user->member_tier == 'Bac')
                                <span class="bg-slate-100 text-slate-600 px-2 py-1 rounded text-xs font-bold"><i class="fa-solid fa-medal mr-1"></i>Bạc</span>
                            @else
                        ...
                        <td class="p-4 text-center">
                            ...
                            @if(Auth::user()->user_id != $user->user_id)
                                <form method="POST" action="{{ route('admin.users.destroy', $user->user_id) }}" class="inline-block" onsubmit="return confirmDelete(event)">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                                        class="text-red-500 hover:bg-red-50 w-8 h-8 rounded-full transition" title="Xóa">
...
<div id="userModal" class="hidden fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto relative">
        ...
        <form id="userForm" method="POST" action="{{ route('admin.users.store') }}" class="p-6 space-y-4">
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">
                    <i class="fa-solid fa-lock mr-1 text-red-500"></i> Mật khẩu <span id="pwdRequired" class="text-red-500">*</span>
                </label>
