@php
    ...
    $userRoleId = \Auth::check() ? (int) \Auth::user()->role_id : 0;

    // Danh sách menu đầy đủ với phân quyền theo role_id
    // roles: [1] = Admin only, [1,2] = Admin + Quản lý, [1,2,4] = Tất cả nhân sự
    $allMenu = [
        [
    ...
    // Lọc menu theo role_id hiện tại
    $menu = array_values(array_filter($allMenu, fn($item) => in_array($userRoleId, $item['roles'])));
