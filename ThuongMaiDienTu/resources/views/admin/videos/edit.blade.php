...
<div class="video-upload-card">
    <div class="video-upload-header d-flex justify-content-between align-items-center gap-3">
        <div>
            <h1 class="h4 fw-bold mb-1">Sửa video</h1>
            <div class="text-slate-500">Chỉnh sửa thông tin video #{{ $video->id }} — {{ $video->title }}</div>
        </div>
        <a href="{{ route('admin.videos.index') }}" class="btn btn-light rounded-3 px-3"><i class="fa-solid fa-arrow-left me-1"></i> Quay lại</a>
    </div>

    <div class="p-4 p-md-5">
        <form id="editVideoForm" action="{{ route('admin.videos.update', $video) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="row g-4 g-lg-5">
                ...
                <div class="col-lg-6 pe-lg-5 col-lg-6 border-end-lg">
                    ...
                    <div class="mb-4">
                        ...
                        @if($video->thumbnail_url)
                            <div class="current-media mb-2">
                                <img src="{{ $video->thumbnail_url }}" alt="current thumbnail">
                                <span>Ảnh hiện tại — chọn ảnh mới bên dưới để thay thế</span>
                            </div>
                        @endif
                        <div class="dropzone-area" id="thumbnailDropzone">
                            ...
                            <div class="dropzone-inner text-center border border-dashed rounded-4 cursor-pointer">
                                ...
                                <div class="fw-bold text-slate-800 text-xs" id="thumbnailInfo">Nhấn để chọn ảnh mới (tùy chọn)</div>
                    ...
                    <div>
                        ...
                        @if($video->video_path)
                            <div class="current-media mb-2">
                                <i class="fa-solid fa-file-video fs-5"></i>
                                <span>Đang có tệp video nội bộ — tải lên tệp mới để thay thế</span>
                            </div>
                        @endif

                        <!-- Source switcher tabs -->
                        <div class="d-flex bg-slate-100 p-1 rounded-3 mb-3 border border-slate-200">
                            <button type="button" id="tab-mp4" class="btn btn-sm flex-fill source-tab {{ !$video->youtube_url || $video->video_path ? 'active' : '' }}">
                                <i class="fa-solid fa-file-video me-1"></i> Tệp MP4 nội bộ
                            </button>
                            <button type="button" id="tab-youtube" class="btn btn-sm flex-fill source-tab {{ $video->youtube_url && !$video->video_path ? 'active' : '' }}">
                                <i class="fa-brands fa-youtube me-1"></i> Đường dẫn YouTube
                            </button>
                        </div>

                        <!-- Panel: Local File Upload -->
                        <div id="panel-mp4" class="video-source-panel {{ $video->youtube_url && !$video->video_path ? 'd-none' : '' }}">
                            <div class="dropzone-area" id="videoDropzone">
                                ...
                                <div class="dropzone-inner text-center border border-dashed rounded-4 cursor-pointer">
                                    ...
                                    <div class="fw-bold text-slate-800 text-xs" id="videoInfo">Nhấn để chọn tệp video mới (tùy chọn)</div>
                                    ...
                                    <div class="validation-badge d-inline-flex py-1 px-2" id="videoValidationBadge" style="font-size: 9px; border-radius: 6px;">
                                        <i class="fa-solid fa-circle-info me-1"></i><span>Bỏ trống nếu không thay đổi</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <!-- Panel: YouTube Link -->
                        <div id="panel-youtube" class="video-source-panel {{ !$video->youtube_url || $video->video_path ? 'd-none' : '' }}">
                            <div class="p-3 border rounded-4 bg-slate-50 d-flex flex-column justify-content-center" style="height: 120px; border-style: dashed !important; border-color: #cbd5e1 !important; border-width: 2px !important;">
                                ...
                                <div class="input-group mb-1">
                                    ...
                                    <input type="text" name="youtube_url" id="youtube_url" class="form-control form-control-premium border-start-0 rounded-end-3 py-1.5 px-3 text-xs" placeholder="https://www.youtube.com/watch?v=..." value="{{ old('youtube_url', $video->youtube_url) }}">
                                </div>
                                <div class="text-slate-400" style="font-size: 10px; line-height: 1.2;">
                                    Bỏ trống nếu không thay đổi đường dẫn YouTube.
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-lg-6 ps-lg-5 d-flex flex-column justify-content-between">
                    <div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="title" class="form-control form-control-premium" placeholder="Nhập tiêu đề giới thiệu video..." value="{{ old('title', $video->title) }}" required>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="category_id" class="form-select form-control-premium">
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category->category_id }}" {{ old('category_id', $video->category_id) == $category->category_id ? 'selected' : '' }}>{{ $category->name }}</option>
                                @endforeach
                            </select>
                        </div>

                        <input type="hidden" name="duration" id="auto_duration" value="{{ $video->duration }}">

                        <div class="mb-3">
                            <label class="form-label fw-semibold text-slate-600 small">Mô tả chi tiết</label>
                            <textarea name="description" class="form-control form-control-premium" rows="4" placeholder="Viết một vài dòng mô tả ngắn về nội dung video này..." style="resize: none;">{{ old('description', $video->description) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="d-flex justify-content-end gap-2 mt-5 pt-4 border-top">
                ...
                <button type="submit" id="editSubmitBtn" class="btn-premium-primary">
...
(function () {
    ...
    thumbnailInput?.addEventListener('change', function () {
        ...
        if (file) {
            ...
            if (thumbnailInfo) thumbnailInfo.textContent = 'Ảnh mới: ' + file.name;
        } else {
            inner.style.backgroundImage = 'none';
            inner.classList.remove('has-preview');
            if (thumbnailInfo) thumbnailInfo.textContent = 'Nhấn để chọn ảnh mới (tùy chọn)';
        }
    });
    ...
    fileInput?.addEventListener('change', function () {
        const file = this.files?.[0];
        if (videoInfo) videoInfo.innerHTML = file ? 'Tệp mới: ' + file.name : 'Nhấn để chọn tệp video mới (tùy chọn)';
        if (!videoValidationBadge) return;
        if (!file) {
            videoValidationBadge.classList.remove('invalid');
            videoValidationBadge.innerHTML = '<i class="fa-solid fa-circle-info me-1"></i><span>Bỏ trống nếu không thay đổi</span>';
            return;
        }
