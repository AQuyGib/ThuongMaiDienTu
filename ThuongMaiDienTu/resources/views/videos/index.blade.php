    ...
    <div class="video-page-shell min-h-screen pb-20">
        @if($videos->isEmpty())
            <div class="container mx-auto px-4 py-16 flex items-center justify-center min-h-[70vh]">
                <div
                    class="text-center px-6 py-12 max-w-lg bg-white rounded-3xl border border-blue-100 shadow-xl mx-4 transform hover:scale-[1.01] transition duration-300">
                    <div
                        class="w-28 h-28 mx-auto mb-6 bg-red-50 rounded-full flex items-center justify-center text-red-500 border border-red-100 relative">
                    ...
                    <p class="text-gray-500 text-sm leading-relaxed mb-6">
                        Hiện tại danh sách video review thiết bị đã được làm trống để cập nhật các nội dung mới. Vui lòng quay
                        lại sau hoặc liên hệ Admin để thêm video review!
                    </p>
                    <div class="flex flex-col sm:flex-row gap-3 justify-center">
                        <a href="{{ url('/') }}"
                            class="inline-flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-primary text-white font-bold hover:bg-blue-800 transition shadow-md">
                            <i class="fa-solid fa-house"></i> Quay về Trang chủ
                        </a>
                        <a href="javascript:void(0)" onclick="chatbotToggle()"
                            class="inline-flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-slate-50 border border-gray-200 text-gray-600 font-semibold hover:bg-slate-100 transition">
            ...
            <main class="container mx-auto px-4 py-6 md:py-10">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
                        <div>
                            ...
                            <p class="text-gray-600 text-sm md:text-base leading-7 max-w-xl">
                                Xem cận cảnh, đánh giá chi tiết các thiết bị điện máy, điện tử thông minh và học các mẹo vặt sử
                                dụng hiệu quả từ chuyên gia hàng đầu của DienMayPro.
                            </p>
                            ...
                            @if($hero_video_1)
                                <div class="video-card p-4 cursor-pointer" onclick="playVideo({{ $hero_video_1->id }})">
                                    <div class="video-thumb mb-3">
                                        ...
                                        <span
                                            class="absolute bottom-2 right-2 bg-black/70 text-white text-[10px] font-bold px-1.5 py-0.5 rounded">
                                    ...
                                    <p class="text-xs text-gray-500 mt-1 line-clamp-2">
                                        {{ $hero_video_1->description ?: 'Không có mô tả' }}</p>
                                </div>
                            @endif

                            @if($hero_video_2)
                                <div class="video-card p-4 mt-0 sm:mt-8 cursor-pointer"
                                    onclick="playVideo({{ $hero_video_2->id }})">
                                    <div class="video-thumb mb-3">
                                    ...
                                    <p class="text-xs text-gray-500 mt-1 line-clamp-2">
                                        {{ $hero_video_2->description ?: 'Không có mô tả' }}</p>
                                </div>
                            @endif
                        </div>
                    </div>
                ...
                <div class="mb-8 relative z-10">
                    ...
                    <div class="relative">
                        ...
                        <div
                            class="absolute top-0 right-0 bottom-0 w-12 bg-gradient-to-l from-[#f8fafc] to-transparent pointer-events-none z-10">
                        </div>
                        <div
                            class="absolute top-0 left-0 bottom-0 w-6 bg-gradient-to-r from-[#f8fafc] to-transparent pointer-events-none z-10">
                        </div>

                        <div id="category-filter-scroll"
                            class="flex items-center gap-2 overflow-x-auto pb-2 pr-14 custom-scrollbar">
                            <button onclick="filterCategory('all')" id="cat-tab-all"
                                class="category-filter-tab px-5 py-2.5 rounded-full font-bold text-xs transition-all duration-300 shadow-sm bg-primary text-white border border-transparent whitespace-nowrap shrink-0">
                                <i class="fa-solid fa-layer-group mr-1.5"></i> Tất cả
                            </button>
                            @foreach($all_categories as $key => $name)
                                <button onclick="filterCategory('{{ $key }}')" id="cat-tab-{{ $key }}"
                                    class="category-filter-tab px-5 py-2.5 rounded-full font-bold text-xs transition-all duration-300 shadow-sm bg-white text-gray-600 hover:bg-slate-50 border border-gray-150 whitespace-nowrap shrink-0">
                                    {{ $name }}
                                </button>
                            @endforeach
                        </div>
                ...
                <section id="featured-video" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div class="video-card p-4 md:p-6 flex flex-col justify-between">
                            ...
                            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
                                <div class="flex items-center gap-2 flex-wrap">
                                    <span id="player-category"
                                        class="px-2.5 py-1 rounded-full bg-blue-50 text-primary text-xs font-bold uppercase tracking-wider">
                                        {{ $featured_video->category_name }}
                                    </span>
                                    <span
                                        class="px-2.5 py-1 rounded-full bg-red-50 text-danger text-xs font-bold animate-pulse">
                                        🔥 Đang chiếu
                                    </span>
                                </div>
                                <!-- BỘ CHUYỂN NGUỒN PHÁT DỰ PHÒNG -->
                                <div id="source-switcher"
                                    class="flex items-center gap-1 bg-slate-100 p-1 rounded-xl self-start text-[11px] font-bold text-gray-700 {{ (!$hasMp4 || !$hasYoutube) ? 'hidden' : '' }}">
                                    <span class="px-1.5 text-gray-500 uppercase text-[9px]">Nguồn:</span>
                                    <button id="src-mp4-btn" onclick="switchSource('mp4')"
                                        class="px-2.5 py-1 rounded-lg {{ $defaultSource === 'mp4' ? 'bg-primary text-white shadow-sm active' : 'hover:bg-white text-primary' }} source-switcher-btn">
                                        <i class="fa-solid fa-file-video mr-1"></i> MP4
                                    </button>
                                    <button id="src-yt-btn" onclick="switchSource('youtube')"
                                        class="px-2.5 py-1 rounded-lg {{ $defaultSource === 'youtube' ? 'bg-primary text-white shadow-sm active' : 'hover:bg-white text-red-600' }} source-switcher-btn">
                            ...
                            <div class="video-player-frame mb-4">
                                ...
                                <div id="mp4-player-container"
                                    class="w-full h-full {{ $defaultSource === 'youtube' ? 'hidden' : '' }}">
                                    <video id="main-video-player" class="w-full h-full object-cover animate-fade-in" controls
                                        preload="auto" poster="{{ $getThumbUrl($featured_video->thumbnail_path) }}"
                                        src="{{ $getVideoUrl($featured_video) }}">
                                ...
                                <div id="youtube-player-container"
                                    class="w-full h-full {{ $defaultSource === 'youtube' ? '' : 'hidden' }}">
                                    <iframe id="main-youtube-player" class="w-full h-full border-0 absolute inset-0"
                                        src="{{ $defaultSource === 'youtube' ? ($featured_video->youtube_url . (str_contains($featured_video->youtube_url, '?') ? '&' : '?') . 'rel=0') : '' }}"
                            ...
                            <div class="flex-grow flex flex-col justify-between">
                                <div>
                                    <div class="flex justify-between items-start gap-4">
                                        <h2 id="player-title"
                                            class="text-xl md:text-2xl font-extrabold text-gray-900 leading-tight">
                                            {{ $featured_video->title }}
                                        </h2>
                                        <span id="player-views" class="text-xs text-gray-500 font-medium shrink-0 mt-1">
                                            <i class="fa-solid fa-eye text-primary mr-1"></i>
                                            {{ number_format($featured_video->views) }} lượt xem
                                        </span>
                                    </div>
                                ...
                                <div
                                    class="mt-6 pt-4 border-t border-gray-100 flex items-center justify-between text-xs text-gray-500">
                                    <div class="flex items-center gap-1">
                                        <i class="fa-solid fa-clock"></i>
                                        <span id="player-date">Cập nhật ngày:
                                            {{ optional($featured_video->published_at ?: $featured_video->created_at)->format('d/m/Y') }}</span>
                                    </div>
                                    <div class="flex items-center gap-3">
                                        <!-- NÚT THÍCH SINH ĐỘNG -->
                                        <button id="like-btn" onclick="toggleLikeCurrentVideo()"
                                            class="flex items-center gap-1.5 px-4 py-1.5 rounded-full bg-slate-100 hover:bg-slate-200 text-gray-700 font-bold transition-all relative overflow-visible group">
                                            <i id="like-icon"
                                                class="fa-regular fa-heart text-red-500 group-hover:scale-110 transition-transform duration-200"></i>
                                            <span id="like-text" style="font-weight: 700;">Thích</span>
                                            <span id="like-count"
                                                class="bg-white px-2 py-0.5 rounded-full text-[10px] text-gray-500 border border-gray-200/50 shadow-sm font-extrabold">{{ $featured_video->likes }}</span>
                                        </button>

                                        <!-- NÚT CHIA SẺ -->
                                        <button id="share-btn" onclick="shareCurrentVideo()"
                                            class="flex items-center gap-1.5 px-4 py-1.5 rounded-full bg-slate-100 hover:bg-slate-200 text-gray-700 font-bold transition-all">
                        ...
                        <div class="video-card p-5 md:p-6">
                            <h3 class="font-extrabold text-gray-900 text-base mb-4 flex items-center gap-2">
                                ...
                                <span id="comment-count-badge"
                                    class="text-xs font-bold text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">0</span>
                            </h3>
                            ...
                            <div class="mb-6">
                                @auth
                                    <form id="comment-form" onsubmit="submitComment(event)" class="flex gap-3 items-start">
                                        <div
                                            class="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center font-bold text-primary shrink-0 border border-primary/20">
                                            {{ mb_substr(auth()->user()->full_name ?? auth()->user()->name ?? 'U', 0, 1) }}
                                        </div>
                                        <div class="flex-grow">
                                            <textarea id="comment-textarea" placeholder="Viết bình luận của bạn..."
                                                class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition resize-none h-20"
                                                required></textarea>
                                            <div class="flex justify-end mt-2">
                                                <button type="submit"
                                                    class="px-4 py-2 bg-primary text-white rounded-xl text-xs font-bold hover:bg-blue-800 transition shadow-sm flex items-center gap-1.5">
                                ...
                                @else
                                    <div
                                        class="bg-slate-50 border border-gray-150 rounded-xl p-4 text-center text-sm text-gray-500">
                                        Vui lòng <a href="{{ route('login') }}" class="text-primary font-bold hover:underline">Đăng
                                            nhập</a> để tham gia bình luận.
                                    </div>
                                @endauth
                            </div>
                    ...
                    <aside class="space-y-4">
                        <div class="video-playlist-container p-4">
                            <h3
                                class="font-extrabold text-gray-900 text-base mb-3 pb-3 border-b border-gray-100 flex items-center justify-between">
                                <span class="flex items-center gap-2">
                                    <i class="fa-solid fa-list-ul text-primary"></i> Playlist bài viết
                                </span>
                                <span id="playlist-video-count"
                                    class="text-xs font-bold text-gray-400 bg-gray-155 px-2.5 py-0.5 rounded-full">
                                    {{ $videos->count() }} Video
                                </span>
                            </h3>
                            ...
                            <div class="playlist-scroll space-y-2 pr-1 flex flex-col gap-2">
                                @foreach($videos as $v)
                                    ...
                                    @endphp
                                    <button id="playlist-item-{{ $v->id }}" onclick="playVideo({{ $v->id }})"
                                        data-category-id="{{ $v->getRootCategory() ? $v->getRootCategory()->category_id : $v->category_id }}"
                                        data-parent-id="{{ $parentId }}" data-category-name="{{ $v->category_name }}"
                                        data-video-id="{{ $v->id }}"
                                        class="w-full text-left playlist-item p-2.5 flex gap-3 items-center rounded-xl hover:bg-slate-50 transition-all border border-gray-100 {{ $v->id == $featured_video->id ? 'active' : '' }}">
                                        <div
                                            class="w-24 h-14 rounded-lg overflow-hidden shrink-0 bg-blue-50 relative shadow-sm border border-gray-200/40">
                                            <img src="{{ $getThumbUrl($v->thumbnail_path) }}" class="w-full h-full object-cover"
                                                alt="thumbnail">
                                            <span
                                                class="absolute bottom-1 right-1 bg-black/70 text-white text-[9px] font-bold px-1 py-0.5 rounded leading-none">
                                                {{ $v->duration ?: '0:00' }}
                                            </span>
                                        </div>
                                        <div class="flex-1 min-w-0">
                                            <span
                                                class="text-[9px] font-bold text-primary uppercase tracking-wider block leading-none mb-1">
                                                {{ $v->category_name }}
                                            </span>
                                            <h4 class="font-extrabold text-gray-800 text-xs line-clamp-2 leading-snug">
                                                {{ $v->title }}</h4>
                                            <div class="flex items-center gap-1.5 mt-1">
                                                <span class="text-[10px] text-gray-400"><i class="fa-solid fa-eye text-[9px]"></i>
                                                    {{ number_format($v->views) }}</span>
                                                <span class="text-[10px] text-gray-400 ms-2"><i
                                                        class="fa-solid fa-heart text-red-500 text-[9px]"></i>
                                                    <span>{{ number_format($v->likes) }}</span></span>
                        ...
                        <div
                            class="video-card p-5 bg-gradient-to-br from-blue-50 to-white flex flex-col justify-between h-48 border border-blue-100">
                            <div>
                                ...
                                <p class="text-xs text-gray-600 leading-relaxed mt-2">
                                    Mọi sản phẩm của Điện Máy Pro đều được đội ngũ kỹ thuật viên review chân thực và hướng dẫn
                                    sử dụng chi tiết nhất giúp bạn hoàn toàn an tâm mua sắm.
                                </p>
                            </div>
                            <a href="{{ url('/') }}"
                                class="inline-flex items-center justify-center gap-2 w-full py-2.5 rounded-xl bg-primary text-white text-xs font-bold hover:bg-blue-800 transition shadow-md">
