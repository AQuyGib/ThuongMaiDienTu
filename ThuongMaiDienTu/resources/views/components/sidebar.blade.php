...
<aside id="sidebar"
    class="w-64 bg-slate-900 text-white flex flex-col h-screen fixed md:sticky top-0 left-0 z-50 shrink-0 transition-transform duration-300 -translate-x-full md:translate-x-0">
    ...
    <div class="h-16 flex items-center justify-between px-4 border-b border-slate-700">
        <a href="{{ route('dashboard') }}"
            ...
            <i class="fa-solid fa-bolt-lightning"></i> ĐIỆN MÁY PRO
        </a>
    ...
    <nav class="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
        ...
        <a href="{{ route('cashbooks.index') }}"
            class="flex items-center gap-3 px-4 py-3 rounded-lg transition
                   {{ request()->routeIs('cashbooks*') ? 'bg-indigo-600 text-white' : 'text-slate-300 hover:bg-slate-800' }}">
    ...
    <div class="p-4 border-t border-slate-700 text-sm shrink-0">
        <div class="flex items-center gap-3 mb-4 text-slate-300">
            ...
            <div>
                @auth
                    <div class="font-bold text-white">{{ Auth::user()->full_name ?? Auth::user()->name ?? 'Admin' }}</div>
                    <div class="text-xs text-green-400">{{ Auth::user()->role->name ?? 'Administrator' }}</div>
                @else
        ...
        <a href="/"
            class="block w-full text-center py-2 bg-slate-800 hover:bg-slate-700 rounded transition">
