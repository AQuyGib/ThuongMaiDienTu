...
@push('styles')
<!-- CSS TÙY CHỈNH CHO TRANG SO SÁNH SẢN PHẨM (COMPARE SYSTEM) -->
<style>
...
/* === EMPTY STATE: Trạng thái khi không có sản phẩm nào để so sánh === */
...
/* === STICKY PRODUCT HEADER: Thanh tiêu đề sản phẩm ghim phía trên màn hình === */
...
/* Nút xóa sản phẩm khỏi khay so sánh nhanh */
...
/* Ô trống hỗ trợ tìm kiếm & thêm trực tiếp sản phẩm so sánh */
...
/* === COMPACT STICKY HEADER: Thu nhỏ thanh sản phẩm khi cuộn xuống dưới === */
...
/* Cấu trúc dropdown kết quả tìm kiếm sản phẩm inline */
...
/* === TOGGLE BAR: Thanh chuyển đổi bộ lọc thông số === */
...
/* === SPECS TABLE: Bảng thông số kỹ thuật động === */
...
/* Nổi bật dòng thông số có sự khác biệt giữa các sản phẩm để người dùng dễ nhận biết */
...
/* Lớp dùng để ẩn dòng khi kích hoạt bộ lọc "Chỉ xem điểm khác biệt" */
...
/* Toast alert dạng trượt góc màn hình */
...
<div class="container cmp-page">
    ...
    <!-- TRẠNG THÁI TRỐNG: Chưa chọn sản phẩm nào để so sánh -->
    @if($products->isEmpty())
        <div class="cmp-empty">
    ...
    @else
        ...
        <!-- Đặt tổng số cột hiển thị cho Grid CSS động dựa trên số sản phẩm so sánh thực tế -->
        <div class="cmp-wrapper" style="--cols: {{ $cols }};">

            {{-- Sticky Header: Panel sản phẩm luôn nổi khi cuộn trang --}}
            <div class="cmp-sticky">
                <div class="cmp-header">
                    <div class="cmp-header-label">
                        <i class="fa-solid fa-scale-balanced" style="margin-right:6px;color:#0046ab"></i> Sản phẩm
                    </div>
                    
                    <!-- Hiển thị thông tin tóm tắt các sản phẩm đang so sánh -->
                    @foreach($products as $p)
                        <div class="cmp-prod-card">
                            <!-- Nút loại bỏ sản phẩm -->
                            <button class="cmp-remove" onclick="cmpRemoveReload({{ $p->product_id }})" title="Xóa">
                            ...
                            <button class="cmp-buy-btn" onclick="addToCart({{ $p->product_id }})">
                                <i class="fa-solid fa-cart-plus"></i> Thêm vào giỏ
                            </button>
                        </div>
                    @endforeach

                    <!-- Nếu số sản phẩm hiện tại ít hơn 3, hiển thị các ô trống cho phép tìm và thêm trực tiếp -->
                    @for($i = $cols; $i < 3; $i++)
                        <div class="cmp-add-slot" onclick="cmpOpenSearch(this)">
                            <i class="fa-solid fa-plus cmp-add-icon"></i>
                            <span>Thêm sản phẩm</span>
                            <!-- Form tìm kiếm sản phẩm inline bằng AJAX -->
            ...
            {{-- Toggle lọc xem các thuộc tính khác biệt giữa các sản phẩm --}}
            <div class="cmp-toggle">
                ...
                <div class="cmp-toggle-right">
                    ...
                    <label class="tgl">
                        <input type="checkbox" id="tglDiff" onchange="cmpToggle()">
                        <span class="tgl-slider"></span>
                    </label>
                </div>
            </div>

            {{-- Bảng hiển thị thông số kỹ thuật chi tiết được render tự động --}}
            <div class="cmp-table">
                @forelse($comparisonData as $row)
                    <div class="cmp-row {{ $row['is_different'] ? 'diff' : '' }}" data-diff="{{ $row['is_different'] ? '1' : '0' }}">
                        ...
                        @endforeach
                        <!-- Đảm bảo căn chỉnh số cột đồng đều nếu so sánh ít hơn 3 sản phẩm -->
                        @for($i = count($row['values']); $i < $cols; $i++)
...
<!-- Khung Toast thông báo đẩy của phân hệ so sánh -->
<div class="cmp-toast" id="cmpToast"><i></i><span id="cmpToastMsg"></span></div>
@endsection

@push('scripts')
<script>
// Token CSRF bảo mật bắt buộc của Laravel
const CMP_CSRF='{{ csrf_token() }}';
// ID danh mục hiện hành (để giới hạn chỉ cho phép so sánh các sản phẩm cùng loại)
const CMP_CAT={{ $products->first()->category_id ?? 'null' }};
// Danh sách các ID sản phẩm đã có mặt trong khay so sánh để loại trừ khỏi kết quả tìm kiếm
const CMP_EX={!! json_encode($products->pluck('product_id')->toArray()) !!};

/**
 * Hàm: cmpToggle
 * Công dụng: Bật/tắt bộ lọc chỉ hiển thị các dòng thông số kỹ thuật có sự khác nhau.
 * Cách chạy: Thêm/xóa lớp CSS 'hidden-row' đối với các dòng có thuộc tính data-diff="0" (tức giống nhau).
 */
function cmpToggle(){
...
/**
 * Hàm: cmpRemoveReload
 * Công dụng: Gửi yêu cầu xóa một sản phẩm khỏi danh sách so sánh trong Session và reload lại trang.
 */
function cmpRemoveReload(id){
    fetch(`/compare/remove/${id}`,{
        method:'DELETE',
        headers:{
            'X-CSRF-TOKEN':CMP_CSRF,
...
/**
 * Hàm: cmpOpenSearch
 * Công dụng: Kích hoạt hiển thị ô tìm kiếm sản phẩm ngay bên trong ô thêm sản phẩm.
 */
...
// Biến lưu trữ Timeout ID phục vụ Debounce tìm kiếm
let cmpST;

/**
 * Hàm: cmpSearch
 * Công dụng: Tìm kiếm sản phẩm cùng danh mục bằng AJAX có tích hợp cơ chế Debounce chống overload API.
 */
function cmpSearch(inp){
    ...
    cmpST=setTimeout(()=>{
        ...
        fetch(u).then(r=>r.json()).then(ps=>{
            rb.innerHTML=ps.length?ps.map(p=>`
                <div class="cmp-sr-item" onclick="cmpAdd(${p.product_id})">
                    <img src="${p.thumbnail||'https://via.placeholder.com/40'}">
                    <span class="cmp-sr-item-name">${p.name}</span>
                    <span class="cmp-sr-item-price">${Number(p.base_price).toLocaleString('vi-VN')}đ</span>
                </div>`).join(''):'<div style="padding:16px;text-align:center;color:#aaa;font-size:13px">Không tìm thấy</div>';
...
/**
 * Hàm: cmpAdd
 * Công dụng: Gửi yêu cầu POST thêm sản phẩm vào danh sách so sánh và tải lại trang để hiển thị cột mới.
 */
function cmpAdd(id){
    fetch('/compare/add',{
        method:'POST',
    ...
    .then(r=>r.json()).then(d=>{
        ...
        else cmpToast(d.message,'err');
    });
}

/**
 * Hàm: addToCart
 * Công dụng: Thêm sản phẩm vào giỏ hàng từ trang so sánh thông qua AJAX.
 */
function addToCart(productId) {
    ...
    .then(data => {
        if (data.success || data.status === 'ok') {
            cmpToast('Đã thêm sản phẩm vào giỏ hàng thành công!', 'ok');
            if (typeof updateCartCount === 'function') updateCartCount();
        } else {
            cmpToast(data.message || 'Lỗi thêm vào giỏ hàng!', 'err');
        }
    })
    .catch(err => {
        cmpToast('Có lỗi xảy ra, vui lòng thử lại.', 'err');
    });
}

/**
 * Hàm: cmpToast
 * Công dụng: Hiển thị Toast thông báo trạng thái góc màn hình.
 */
function cmpToast(msg,t='ok'){
    const el=document.getElementById('cmpToast');
    el.className='cmp-toast '+t;
    el.querySelector('i').className=t==='ok'?'fa-solid fa-circle-check':'fa-solid fa-circle-exclamation';
    document.getElementById('cmpToastMsg').textContent=msg;
    el.classList.add('show');
    setTimeout(()=>el.classList.remove('show'),3000);
}

// Lắng nghe sự kiện click bên ngoài để tự động đóng dropdown tìm kiếm sản phẩm so sánh
document.addEventListener('click',e=>{
    if(!e.target.closest('.cmp-inline-search')&&!e.target.closest('.cmp-add-slot')){
...
// Lắng nghe sự kiện cuộn trang để thu nhỏ Panel sản phẩm (Compact Mode) giúp tối ưu diện tích xem bảng dữ liệu
