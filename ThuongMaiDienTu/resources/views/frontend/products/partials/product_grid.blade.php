<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
    @forelse($products as $product)
        <div class="product-card group relative bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 border border-gray-100">
            <!-- Badges -->
            ...
            <div class="relative h-48 overflow-hidden bg-gray-100">
                ...
                <div class="absolute top-3 right-3 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    <button class="w-8 h-8 bg-white rounded-full flex items-center justify-center shadow-md hover:bg-red-500 hover:text-white transition-colors" title="Yêu thích">
                        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                            <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/>
                        </svg>
                    </button>
                    <button class="w-8 h-8 bg-white rounded-full flex items-center justify-center shadow-md hover:bg-blue-500 hover:text-white transition-colors" title="So sánh">
            ...
            <div class="p-4">
                ...
                <div class="flex flex-wrap gap-1 mb-3">
                    @if($product->ram)
                        <span class="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded">{{ $product->ram }}</span>
                    @endif
                    @if($product->rom)
                        <span class="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded">{{ $product->rom }}</span>
                    @endif
                    @if($product->screen)
                        <span class="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded">{{ $product->screen }}</span>
                    @endif
                </div>
                ...
                <div class="flex gap-2">
                    ...
                    <button class="flex-1 bg-gray-100 text-gray-700 py-2 rounded-lg text-sm font-medium hover:bg-gray-200 transition-colors">
                        Mua ngay
                    </button>
                </div>
            </div>
