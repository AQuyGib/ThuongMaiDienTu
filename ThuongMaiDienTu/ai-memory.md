...
- **Phân hệ Video Admin & Frontend:**
    ...
    - Phát triển tính năng **Bình luận video (Video Comments)**:
        - Tạo migration và chạy thành công để sinh bảng `video_comments` liên kết với bảng `videos` (qua `video_id`) và bảng `users` (qua `user_id` kiểu unsignedInteger khớp với khóa chính `user_id`).
